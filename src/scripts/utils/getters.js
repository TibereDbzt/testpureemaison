...
const getDistanceTo = (el, elSide, target = window, targetSide = 'bottom') => {
    ...
    else if (elSide === 'left' && targetSide === 'right') return Math.ceil((targetBounds.left - elBounds.right));
    else if (elSide === 'left' && targetSide === 'left') return Math.ceil((targetBounds.left - elBounds.left));
