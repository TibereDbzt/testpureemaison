...
export class HeroSection {
    ...
    animateScrollButton() {
        const timeline = gsap.timeline();
        timeline.fromTo(this.DOM.scrollButton.container, { translateY: getDistanceTo(this.DOM.scrollButton.container, 'bottom') + 40, scaleY: 1.6, scaleX: 0.8 }, { translateY:0, scaleX: 1.2, scaleY: 1.2, duration: 1.6, ease: EASES.markedOut });
    ...
    animateIn() {
        this.timeline.add(this.background.animateOpacity());
        this.timeline.add(this.background.animateClipPath(), '>+=1.5');
        this.timeline.add(this.animateTitle(), '<+=0.5');
        this.timeline.add(this.animateTexts(), '<+=0.5');
        this.timeline.add(this.animateScrollButton(), '<');
